...
function findBeaconinLine(beaconData, limit) {

  for (let i = 0; i <= limit; i++) {
    const unusedIntervals = getUnusedPositions(beaconData, i);

    if (unusedIntervals.length > 1) {
      return [i, unusedIntervals[0][1] + 1];
    }
  }
}
...
const foundBeacon = findBeaconinLine(beaconData, LIMIT);

console.log(foundBeacon);
console.log(foundBeacon[1] * 4000000 + foundBeacon[0]);
